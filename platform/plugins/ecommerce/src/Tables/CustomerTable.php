...
class CustomerTable extends TableAbstract
{
    ...
    public function ajax()
    {
        $data = $this->table
            ...
            ->editColumn('wallet', function ($item) {
                return clean(number_format($item->wallet));
            })
    ...
    {
        $button = $this->addCreateButton(route('customers.create'), 'customers.create');
        $button['extend'] = [
            'link' => route('customers.reset-point-customer'),
            'text' => '<i class="fas fa-receipt"></i> Reset điểm',
            'className'=>'aaa'
        ];
        $button['extends'] = [
            'link' => route('customers.share-wallet-all'),
            'text' => '<i class="fas fa-money-check-alt"></i> Chia điểm',
            'className'=>'chia-diem deleteDialog'
        ];
        return $button;
    }
