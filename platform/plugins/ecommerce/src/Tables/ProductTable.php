    ...
    public function ajax()
    {
        $data = $this->table
            ...
            ->editColumn('tax_id', function ($item) {
                return $item->tax->percentage.'%';
            })
            ->editColumn('price_with_tax', function ($item) {
                $tax = $item->price * ($item->tax->percentage/100);
                $prices = $item->price;
                $totalPrice = $tax+$prices;
                return number_format(ceil($item->price_with_taxes/100)*100);
            })
