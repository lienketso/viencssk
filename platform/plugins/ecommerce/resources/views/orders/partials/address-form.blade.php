        ...
        <div class="row">
            <div class="col-sm-6 col-12">
                <div class="form-group @if ($errors->has('address[city]')) has-error @endif">
                    <label for="address[city]" class="required">Tỉnh/Thành phố:</label>
                    <select name="address[city]" class="form-control square" id="address_city" data-ajax="{{route('public.ajax.get-districts')}}">
                        @foreach(['' => 'Chọn Tỉnh/Thành phố'] + EcommerceHelper::getAvailableProvinces() as $provinceId => $provinceName)
                            <option value="{{ $provinceName }}" data-id="{{$provinceId}}" @if (old('address[city]', Arr::get($sessionCheckoutData, 'city')) == $provinceName) selected @endif>{{ $provinceName }}</option>
                        @endforeach
                    </select>
            ...
            <div class="col-sm-6 col-12">
                <div class="form-group @if ($errors->has('address.state')) has-error @endif">
                    <label for="address[state]" class="required">Quận/Huyện:</label>
                    <select name="address[state]" class="form-control square" id="address_state" data-ajax="{{route('public.ajax.get-wards')}}">
                        @foreach(['' => 'Chọn Quận/Huyện'] + EcommerceHelper::getAvailableDistrict(old('address[city]')) as $districtId => $districtName)
                            <option value="{{ $districtName }}" data-id="{{$districtId}}" @if (old('address.state') == Arr::get($sessionCheckoutData, 'state')) selected @endif>{{ $districtName }}</option>
                        @endforeach
                    </select>
            ...
            <div class="col-sm-6 col-12">
                <div class="form-group @if ($errors->has('address.state')) has-error @endif">
                    <label for="address[ward]" class="required">Phường/Xã:</label>
                    <select name="address[ward]" class="form-control square" id="address_ward">
                        @foreach(['' => 'Chọn Phường/Xã'] + EcommerceHelper::getAvailableDistrict(old('ward')) as $districtId => $districtName)
                            <option value="{{ $districtName }}" data-id="{{$districtId}}" @if (old('address.ward') == Arr::get($sessionCheckoutData, 'ward')) selected @endif>{{ $districtName }}</option>
                        @endforeach
                    </select>
                    {!! Form::error('address.state', $errors) !!}
                </div>
            ...
            <div class="col-sm-6 col-12">
                <div class="form-group @if ($errors->has('address.address')) has-error @endif">
                    <label for="address[address]" class="required">Địa chỉ:</label>
