...
{{--                                    <p class="price-text sub-total-text text-end"> {{ format_price(Cart::instance('cart')->rawSubTotal()) }} </p>--}}
                                    <p class="price-text sub-total-text text-end"> {{ format_price(Cart::instance('cart')->rawSubTotal()+Cart::instance('cart')->rawTax()) }} </p>
                                </div>
                            </div>
...
{{--                            @endif--}}
                            ...
                            @if ($payWithUbgXu)
                                @php
                                    $prePay = 0;
                                    if (auth('customer')->user()->ubgxu >= $subTotal) {
                                        $prePay = $subTotal;
                                    } else {
                                        $prePay = auth('customer')->user()->ubgxu;
                                    }
                                @endphp
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-6 float-end">
                                        <p class="price-text shipping-price-text">-{{ format_price($prePay) }}</p>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="row">
                                ...
                                <input
                                    type="hidden"
                                    id="shopeepay-total-price"
                                    value="{{ format_price(Cart::instance('cart')->rawTotal(), null, true) }}"
                                >

                                <input type="hidden" name="order_shipping_value" value="{{ $shippingAmount }}">
                                <input type="hidden" name="order_shipping_calculate" value="1">

                                @php
                                    if ($payWithUbgXu) {
                                        if (auth('customer')->user()->ubgxu >= $subTotal) {
                                            $totalPrice = ($promotionDiscountAmount + $couponDiscountAmount - $shippingAmount) > Cart::instance('cart')->rawTotal() ? 0 : Cart::instance('cart')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount - $subTotal;
                                        } else {
                                            $totalPrice = ($promotionDiscountAmount + $couponDiscountAmount - $shippingAmount) > Cart::instance('cart')->rawTotal() ? 0 : Cart::instance('cart')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount - auth('customer')->user()->ubgxu;
                                        }
                                    } else {
                            ...
                            <div class="position-relative">
                                ...
                                <div id="ubgxu-checkout" class="ubgxu-checkout">
                                    ...
                                    <div class="ubgxu-checkout-content">
                                        <div class="ubgxu-checkout-title">
                                            <div class="custome-checkbox">
                                                <input type="checkbox" name="ubg_xu_checkout" id="ubg-xu-checkout" @if($payWithUbgXu) checked @endif class="form-check-input" {{auth('customer')->user()->ubgxu == 0 ? 'disabled' : ''}}>
                                        ...
                                        <div class="ubgxu-checkout-avaiable">
                                            <p>Số điểm khả dụng:
                                                @if ($payWithUbgXu)
                                                    @php
                                                        $xu = auth('customer')->user()->ubgxu;
                                                        if (auth('customer')->user()->ubgxu >= $subTotal) {
                                                            $xu = auth('customer')->user()->ubgxu - $subTotal;
                                                        } else {
                                                            $xu = 0;
                                                        }
                                                    @endphp
                                                    <b>{{number_format($xu)}} điểm</b>
                                                @else
                                                    <b>{{number_format(auth('customer')->user()->ubgxu)}} điểm</b>
                                                @endif
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <ul class="list-group list_payment_method">
                                    ...
                                    @endif
{{--                                    <li class="list-group-item">--}}
{{--                                        <input type="hidden" id="ajax-vnpay-request" value="{{route('public.checkout.ajax-vnpay-request', $token)}}">--}}
{{--                                        <input class="magic-radio js_payment_method" type="radio" name="payment_method" id="payment_vnpay" value="vnpay"--}}
{{--                                               data-bs-toggle="collapse" data-target=".payment_vnpay_wrap" data-parent=".list_payment_method">--}}

{{--                                        @if (Request::get('vnp_ResponseCode') == '00')--}}
{{--                                            <input type="hidden" name="vnp_Amount" value="{{Request::get('vnp_Amount')}}">--}}
{{--                                            <input type="hidden" name="vnp_BankCode" value="{{Request::get('vnp_BankCode')}}">--}}
{{--                                            <input type="hidden" name="vnp_BankTranNo" value="{{Request::get('vnp_BankTranNo')}}">--}}
{{--                                            <input type="hidden" name="vnp_CardType" value="{{Request::get('vnp_CardType')}}">--}}
{{--                                            <input type="hidden" name="vnp_OrderInfo" value="{{Request::get('vnp_OrderInfo')}}">--}}
{{--                                            <input type="hidden" name="vnp_PayDate" value="{{Request::get('vnp_PayDate')}}">--}}
{{--                                            <input type="hidden" name="vnp_ResponseCode" value="{{Request::get('vnp_ResponseCode')}}">--}}
{{--                                            <input type="hidden" name="vnp_TransactionStatus" value="{{Request::get('vnp_TransactionStatus')}}">--}}
{{--                                            <input type="hidden" name="vnp_TxnRef" value="{{Request::get('vnp_TxnRef')}}">--}}
{{--                                            <input type="hidden" name="vnp_TransactionNo" value="{{Request::get('vnp_TransactionNo')}}">--}}
{{--                                            <input type="hidden" name="vnp_OrderShippingValue" value="{{Request::get('vnp_OrderShippingValue')}}">--}}
{{--                                        @endif--}}

{{--                                        <label for="payment_vnpay" class="text-left vnpay_logo">--}}
{{--                                            <img src="{{Theme::asset()->url('imgs/vnpay.png')}}" alt="" class="img-fluid payment-logo" style="width: 80px">--}}
{{--                                        </label>--}}
{{--                                        <div class="payment_vnpay_wrap payment_collapse_wrap collapse" style="padding: 15px 0px;">--}}
{{--                                            <img src="{{Theme::asset()->url('imgs/payment-list.png')}}" alt="" class="img-fluid">--}}
{{--                                        </div>--}}
{{--                                    </li>--}}
                                </ul>
                            </div>
                            ...
                            <div class="form-group mb-3 @if ($errors->has('description')) has-error @endif">
                                <label for="description" class="control-label">Ghi chú đơn hàng</label>
                                <br>
                                <textarea name="description" id="description" rows="3" class="form-control" placeholder="Nhập yêu cầu bạn muốn lưu ý về đơn hàng...">{{ old('description') }}</textarea>
                            ...
                            <div class="form-group mb-3">
                                <div class="thongbao-tt">
                                    <p>Bằng việc đặt hàng, bạn đã đồng ý với
                                        <a target="_blank" href="https://ezlife.vn/terms-conditions">Điều khoản dịch vụ</a> và
                                        <a target="_blank" href="https://ezlife.vn/chinh-sach-bao-mat-thong-tin-nguoi-dung">Chính sách bảo mật</a> của chúng tôi.</p>
                                </div>
                            </div>
                            <div class="form-group mb-3">
                                <div class="row">
        ...
        @if (Request::get('vnp_ResponseCode') == '00')
            <div class="payment-loading">
                <p>Đang xử lý thanh toán đơn hàng...</p>
            </div>
