...
<div class="form-group">
    <div class="storehouse-management">
        <label class="text-title-field">Quản lý kho</label>
...
<div class="storehouse-info">
    <div class="form-group">
        <table class="table table-striped table-bordered table-header-color">
            ...
            @if(isset($storeLocation->getQtyInStore))
                @php
                    $qty = isset($storeLocation->getQtyInStore) ? $storeLocation->getQtyInStore : [];
                @endphp

                @foreach($storeLocation->getStore as $k => $store)
                    <tr>
                        <td>
                            {{$store->name}}
                        </td>
                        <td class="text-right">
                            <input type="text"
                                   ...
                                   value="{{ old('store_qty[]', isset($qty[$k]) ? $qty[$k]->quantity : 0) }}"
                                   name="store_qty[]"
                            >
                        </td>
                    </tr>
                @endforeach
            @else
                @foreach($storeLocation as $k => $store)
                    <tr>
                        <td>
                            {{$store->name}}
                        </td>
