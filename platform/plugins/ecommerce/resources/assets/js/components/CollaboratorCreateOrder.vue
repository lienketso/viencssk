  ...
  <div class="flexbox-grid no-pd-none">
            ...
            <div class="table-wrapper p-none mb20 ps-relative z-index-4" v-if="child_products.length">
              <table class="table-normal">
                ...
                <tr v-for="variant in child_products">
                  ...
                  <td class="pl5 p-r5 width-100-px min-width-100-px text-center">
                    <div class="dropup dropdown-priceOrderNew">
                      <div class="inline_block dropdown">
                        <a class="wordwrap hide-print">{{ parseFloat(Math.round(variant.price / 100) * 100).toLocaleString() }} {{ currency }}</a>
                      </div>
                    </div>
                  </td>
                  ...
                  <td class="pl5 p-r5 width-100-px min-width-100-px text-center">{{ parseFloat(Math.round(variant.price / 100) * 100).toLocaleString() }}
                    {{ currency }}
                  </td>
            ...
            <div class="box-search-advance product">
              <div>
                <input type="text" class="next-input textbox-advancesearch product"
                       :placeholder="__('Tìm kiếm hoặc thêm sản phẩm mới')"
              ...
              <div class="panel panel-default"
                ...
                <div class="panel-body">
                  <div class="box-search-advance-head" v-b-modal.add-product-item>
                    ...
                    <span class="ml10">{{ __('Thêm sản phẩm mới') }}</span>
                  </div>
                    ...
                    <ul class="clearfix" v-show="!loading">
                      <li v-for="product_item in list_products.data"
                        ...
                        <label class="inline_block ml10 mt10 ws-nm"
                            ...
                            product_item.name }} - <strong style="color: #f04124">{{ parseFloat(Math.round(product_item.price / 100) * 100).toLocaleString() }} đ</strong>
          ...
          <div class="row">
            <div class="col-sm-6">
              <div class="form-group mb-3">
                <label class="text-title-field" for="txt-note">{{ __('Ghi chú') }}</label>
                <textarea class="ui-text-area textarea-auto-height" id="txt-note" rows="2"
                          :placeholder="__('Ghi chú cho đơn hàng')" v-model="note"></textarea>
              </div>
            </div>
            ...
            <div class="col-sm-6">`
              <div class="table-wrap">
                <table class="table-normal table-none-border table-color-gray-text text-end">
                  <tbody>
                  <tr>
                    <td class="color-subtext">{{ __('Tổng cộng') }}</td>
                    <td class="pl10">{{ parseFloat(Math.round(child_sub_amount / 100) * 100).toLocaleString()  }} {{ currency }}</td>
                  </tr>
                  <tr>
                    <td>
                      <a href="#" v-b-modal.add-discounts class="hover-underline">
                        <span v-if="!has_applied_discount"><i class="fa fa-plus-circle"></i> {{ __('Thêm giảm giá') }}</span>
                        <span v-if="has_applied_discount">{{ __('Giảm giá')}}</span>
                      </a>
                    ...
                    <td>
                      <a href="#" v-b-modal.add-shipping class="hover-underline">
                                                <span v-if="!child_is_selected_shipping"><i
                                                    class="fa fa-plus-circle"></i> {{ __('Thêm phí vận chuyển') }}</span>
                        <span v-if="child_is_selected_shipping">{{ __('Phí vận chuyển') }}</span>
                      </a>
                  ...
                  <tr class="">
                    ...
                    <td class="">{{ parseFloat(Math.round(child_point_amount / 100) * 100).toLocaleString() }} điểm</td>
                  </tr>
                  <tr class="text-no-bold">
                    <td>{{ __('Tổng thanh toán') }}</td>
                    <td class="pl10">{{ parseFloat(Math.round(child_total_amount / 100) * 100).toLocaleString()  }} {{ currency }}</td>
                  </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div class="col-sm-12">
              <div class="form-group mb-3" v-if="child_customer_id && child_customer">
                <input type="checkbox" name="ubg_xu_checkout" value="on" v-model="isChecked" @change="handleUbgXu(child_customer.ubgxu)">
                <label for="remember-checkbox" class="form-check-label"> <span>Thanh toán trước bằng điểm</span></label>
                <p>Số điểm khả dụng : <strong>{{child_customer.ubgxu}}</strong> điểm</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div class="col-12 col-sm-6 col-md-12 col-lg-6">
              <div class="flexbox-grid-default mt5 mb5">
                ...
                <div class="flexbox-auto-content">
                  <div class="text-upper ws-nm">{{ __('XÁC NHẬN THANH TOÁN VÀ TẠO ĐƠN HÀNG') }}</div>
                </div>
              </div>
            </div>
            <div class="col-12 col-sm-6 col-md-12 col-lg-6 text-end">
              <button class="btn btn-primary" v-b-modal.make-paid
                      :disabled="!child_product_ids.length">{{ __('Thanh toán ngay') }}
              </button>
              <button class="btn btn-primary ml15" v-b-modal.make-pending
                      :disabled="!child_product_ids.length || child_total_amount === 0">{{ __('Trả tiền sau') }}
              </button>
            </div>
          </div>
        </div>
        ...
        <div v-if="!child_customer_id || !child_customer">
          <div class="next-card-section">
            <div class="flexbox-grid-default mb15">
              <div class="flexbox-auto-content">
                <label class="title-product-main">{{ __('Thông tin khách hàng') }}</label>
              </div>
            </div>
              ...
              <div class="box-search-advance customer">
                <div>
                  <input type="text" class="next-input textbox-advancesearch customer"
                         ...
                         :placeholder="__('Tìm kiếm theo số điện thoại hoặc thêm mới')">
                </div>
                <div class="panel panel-default"
                    ...
                    <div class="box-search-advance-head" v-b-modal.add-customer>
                      <div class="flexbox-grid-default flexbox-align-items-center">
                        ...
                        <div class="flexbox-auto-content-right">
                          <span>{{ __('Thêm khách hàng mới') }}</span>
                        </div>
                      </div>
                    </div>
                    <div class="list-search-data">
                      ...
                      <ul class="clearfix" v-show="!loading">
                        ...
                        <li v-if="customers.data.length === 0">
                          <span>{{ __('Không tìm thấy khách hàng') }}</span>
                        </li>
                      </ul>
                    </div>
                  </div>
          ...
          <div class="next-card-section p-none-b">
            <div class="flexbox-grid-default">
              <div class="flexbox-auto-content-left">
                <label class="title-product-main">{{ __('Khách hàng') }}</label>
              </div>
            ...
            <ul class="ws-nm">
              <li>
                ...
                <div class="pull-right color_darkblue mt20">
                  ...
                  <span>{{ child_customer_order_numbers }}</span>
                  {{ __('Đơn hàng') }}
                </div>
              </li>
              <li class="mt10">
          ...
          <div class="next-card-section">
            <ul class="ws-nm">
              <li class="clearfix">
                <div class="flexbox-grid-default">
                  <div class="flexbox-auto-content-left">
                    <label class="title-text-second">{{ __('Địa chỉ nhận hàng')}}</label>
                  </div>
                  <div class="flexbox-auto-left">
              ...
              <li class="text-infor-subdued mt15">
                ...
                <div>
                  <a target="_blank" class="hover-underline"
                     :href="'https://maps.google.com/?q=' + child_customer_address.address + ', ' + child_customer_address.city_name + ', ' + child_customer_address.state_name + ', ' + child_customer_address.country_name + (zip_code_enabled ? ', ' + child_customer_address.zip_code : '')">{{ __('Tìm trên bản đồ') }}</a>
                </div>
              </li>
            </ul>
          </div>
    ...
    <b-modal id="add-product-item" title="Add product" ok-title="Save" cancel-title="Cancel"
      ...
      <div class="form-group mb15">
        <label class="text-title-field">{{ __('Tên sản phẩm') }}</label>
        <input type="text" class="next-input" v-model="product.name">
      </div>
      <div class="form-group mb15 row">
        <div class="col-6">
          <label class="text-title-field">{{ __('Giá sản phẩm') }}</label>
          <input type="text" class="next-input" v-model="product.price">
        </div>
        <div class="col-6">
          <label class="text-title-field">{{ __('Mã sản phẩm (SKU)') }}</label>
      ...
      <div class="row" v-show="product.with_storehouse_management">
        <div class="col-8">
          <div class="form-group mb-3">
            <label class="text-title-field">{{ __('Số lượng') }}</label>
          ...
          <div class="form-group mb-3">
            <label class="next-label">
              <input type="checkbox" class="hrv-checkbox"
                     ...
                     value="1">
              {{ __('Cho phép khách hàng thanh toán khi sản phẩm này hết hàng?') }}</label>
          </div>
        </div>
      </div>
    </b-modal>

    <b-modal id="add-customer" :title="__('Thêm mới khách hàng')" :ok-title="__('Lưu lại')" :cancel-title="__('Bỏ qua')"
      ...
      <div class="next-form-section">
        <div class="next-form-grid">
          <div class="next-form-grid-cell">
            <label class="text-title-field">Tên khách hàng</label>
            <input type="text" class="next-input" v-model="child_customer_address.name">
          </div>
          <div class="next-form-grid-cell">
            <label class="text-title-field">{{ __('Số điện thoại') }}</label>
            <input type="text" class="next-input" v-model="child_customer_address.phone">
          </div>
        </div>
        <div class="next-form-grid">
          <div class="next-form-grid-cell">
            <label class="text-title-field">{{ __('Địa chỉ') }}</label>
        ...
        <div class="next-form-grid">
          <div class="next-form-grid-cell">
            <label class="text-title-field">{{ __('Quốc gia') }}</label>
            <div class="ui-select-wrapper">
        ...
        <div class="next-form-grid">
          <div class="next-form-grid-cell">
            <label class="text-title-field">{{ __('Quận Huyện')}}</label>
          ...
          <div class="next-form-grid-cell">
            <label class="text-title-field">{{ __('Thành phố') }}</label>
            <div class="ui-select-wrapper" v-if="use_location_data">
        ...
        <div class="next-form-grid" v-if="zip_code_enabled">
          <div class="next-form-grid-cell">
            <label class="text-title-field">{{ __('Mã quốc gia') }}</label>
            <input type="text" class="next-input" v-model="child_customer_address.zip_code">
          </div>
        </div>
        <div class="next-form-grid">
          <div class="next-form-grid-cell">
            <label class="text-title-field">Mã giới thiệu ( Nếu có )</label>
            <input type="text" class="next-input customer-address-presenter-id" v-model="child_customer_address.presenter_id">
          </div>
        </div>
      </div>
    </b-modal>

    <b-modal id="add-discounts" title="Add discount" :ok-title="__('Thêm giảm giá')" :cancel-title="__('Đóng')"
             @ok="handleAddDiscount($event)">
      <div class="next-form-section">
        <div class="next-form-grid">
          <div class="next-form-grid-cell">
            <label class="text-title-field">{{ __('Giảm giá dựa trên') }}</label>
            <div class="flexbox-grid-default">
              <div class="flexbox-auto-left">
        ...
        <div class="next-form-grid">
          <div class="next-form-grid-cell">
            <label class="text-title-field">{{ __('Hoặc mã giảm giá') }}</label>
        ...
        <div class="next-form-grid">
          <div class="next-form-grid-cell">
            <label class="text-title-field">{{ __('Mô tả') }}</label>
            <input :placeholder="__('Mô tả về phiếu giảm giá')" class="next-input"
    ...
    <b-modal id="add-shipping" :title="__('Phí vận chuyển')" :ok-title="__('Cập nhật')" :cancel-title="__('Đóng')"
        ...
        <div class="ui-layout__item mb15 p-none-important" v-if="!child_customer_id">
          <div class="ui-banner ui-banner--status-info">
            ...
            <div class="ui-banner__content">
              <h2 class="ui-banner__title">{{ __('Cách lựa chọn cấu hình vận chuyển?') }}</h2>
              <div class="ws-nm">{{ __('Vui lòng thêm thông tin khách hàng cùng địa chỉ giao hàng đầy đủ để xem mức phí vận chuyển được định cấu hình.') }}.
              </div>
            </div>
          </div>
        </div>
    ...
    <b-modal id="edit-email" :title="__('Cập nhật email')" :ok-title="__('Cập nhật')" :cancel-title="__('Đóng')"
    ...
    <b-modal id="edit-address" :title="__('Cập nhật địa chỉ')" :ok-title="__('Lưu lại')" :cancel-title="__('Hủy')"
             @shown="loadCountries()" @ok="updateOrderAddress($event)">
      <div class="next-form-section">
        <div class="next-form-grid">
          <div class="next-form-grid-cell">
            <label class="text-title-field">{{ __('Họ và tên') }}</label>
    ...
    <b-modal id="make-paid" :title="__('Xác nhận thanh toán được thanh toán cho đơn hàng này')" :ok-title="__('Xác nhận ngay')" :cancel-title="__('Hủy')"
             @ok="createOrder($event, true)">
      <div class="note note-warning">
        {{ __('Trạng thái thanh toán của đơn hàng là Đã thanh toán. Khi đơn hàng đã được tạo, bạn không thể thay đổi phương thức hoặc trạng thái thanh toán.') }}.
      </div>
      ...
      <div class="ui-select-wrapper mb15 next-input--is-focused">
        <select class="ui-select" v-model="child_payment_method">
          <option value="cod">{{ __('Thanh toán khi giao hàng') }}</option>
          <option value="bank_transfer">{{ __('Chuyển khoản') }}</option>
        </select>
        <svg class="svg-next-icon svg-next-icon-size-16">
          <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#select-chevron"></use>
        </svg>
      ...
      <br/>
      <p>{{ __('Số tiền thanh toán') }} : <span>{{ parseFloat(Math.round(child_total_amount / 100) * 100).toLocaleString() }} {{ currency }}</span></p>
    </b-modal>

    <b-modal id="make-pending" :title="__('Xác nhận rằng khoản thanh toán cho đơn hàng này sẽ được thanh toán sau')" :ok-title="__('Xác nhận')" :cancel-title="__('Hủy')"
             @ok="createOrder($event)">
      <div class="note note-warning">
        {{ __('Trạng thái thanh toán của đơn hàng là Đang chờ xử lý. Khi đơn hàng đã được tạo, bạn không thể thay đổi phương thức hoặc trạng thái thanh toán.') }}.
      </div>
      <div class="ui-select-wrapper mb15 next-input--is-focused">
        <select class="ui-select" v-model="child_payment_method">
          <option value="cod">{{ __('Thanh toán khi giao hàng') }}</option>
      ...
      <br/>
      <p>{{ __('Số tiền đang chờ xử lý') }} : <span>{{ parseFloat(Math.round(child_total_amount / 100) * 100).toLocaleString() }} {{ currency }}</span></p>
    </b-modal>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    'child_point_amount':function(value){
      let context = this;
