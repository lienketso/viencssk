...
class OrderSupportServiceProvider extends ServiceProvider
{
    ...
    public function processPostCheckoutOrder($products, $request, $token, $sessionCheckoutData, $response)
    {
        ...
        $checkXu = $order->paid_by_ubgxu > 0 ? number_format($order->paid_by_ubgxu).'điểm' : 'Không';
        $text = '<b>🛍 Bạn vừa có đơn hàng mới từ EZLife</b>
        - Mã đơn hàng: '.get_order_code($order['id']).'
        - Người đặt hàng: '.auth('customer')->user()->name.' - '.auth('customer')->user()->phone.' 
        - Giá trị đơn hàng: '.format_price($order["amount"]).'
        - Thanh toán: '.$paymentData["type"].'
        - Vận chuyển: '.$order["shipping_method"].' - '.format_price($order["shipping_amount"]).'
        - Thanh toán dùng điểm: '. $checkXu.' 
        - Link xử lý đơn hàng: '.route("orders.edit", $order->id);
    
//        Telegram::sendMessage([
//            'chat_id' => env('TELEGRAM_CHANNEL_ORDER_ID', ''),
//            'parse_mode' => 'HTML',
//            'text' => $text
//        ]);
    ...
    public function handleCheckoutOrderByStore(
        ...
        $height = 0;
        $weight = 0;
        $storeInfor = $products['store'];
        foreach ($products['products'] as $product) {
            if ($product->weight) {
                $weight += $product->weight * $product->cartItem->qty;
            }
            if ($product->length) {
                $length += $product->length * $product->qty;
            }
            if ($product->height) {
                $height += $product->height * $product->qty;
            }
        }
        ...
        if ($request->has("shipping_method.$storeId") && !get_shipping_setting('free_ship',
            ...
            $loggedInUser = auth('customer')->id();
            $loggedInAddress = app(AddressInterface::class)->getFirstBy(['customer_id' => $loggedInUser]);
            ...
            $shippingData = [
                ...
                'haibonbay_hubid' => $storeInfor->haibonbay_hubid,
            ];
...
//            $shippingAmount = Arr::get(Arr::first($shippingMethod), 'haibonbay.price', 0);
    ...
    public function processPaymentMethodPostCheckout($request, $totalAmount)
    {
        ...
        switch ($request->input('payment_method')) {
            ...
            case PaymentMethodEnum::VNPAY:
                ...
                $payment = app(PaymentInterface::class)->getFirstBy(['order_id' => $orderIds]);
                if ($payment) {
    ...
    public function processShippingDiscountOrderData($products, $token, $sessionCheckoutData, $request)
    {
        ...
        foreach ($groupedProducts as $storeId => $productsByStore) {
            ...
            $length = 0;
            $height = 0;
            $storeInfor = $productsByStore['store'];
            foreach ($cartItems as $cartItem) {
                $productByCartItem = $productsByStore['products']->firstWhere('id', $cartItem->id);
                $weight += $productByCartItem->weight * $cartItem->qty;
                $length += $productByCartItem->length * $cartItem->qty;
                $height += $productByCartItem->height * $cartItem->qty;
            }
...
//            $rules["shipping_method.$storeId"] = 'required|' . Rule::in(ShippingMethodEnum::values());
            $rules["shipping_method.$storeId"] = 'required';
