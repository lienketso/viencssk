...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function registerAdditionalData($form, $data)
    {
        if (get_class($data) == Product::class && request()->segment(1) === BaseHelper::getAdminPrefix()) {
            ...
            if (Route::currentRouteName() != 'ubgxu.transfer.create') {
        ...
        switch (get_class($model)) {
            case Customer::class:
                return $data->addColumn('is_vendor', function ($item) {
                    if (!$item->is_vendor) {
                        return Html::tag('span', trans('core/base::base.no'), ['class' => 'label-warning status-label']);
                    }
                    return Html::tag('span', trans('core/base::base.yes'), ['class' => 'label-info status-label']);
                });
