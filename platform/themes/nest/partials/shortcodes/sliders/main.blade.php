@if (is_plugin_active('simple-slider') && count($sliders) > 0 &&
    ...
    @elseif ($style == 'style-2')
        <section class="home-slider style-2 position-relative mb-20" @if ($shortcode->cover_image) style="background-image: url({{ RvMedia::getImageUrl($shortcode->cover_image) }});" @endif>
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-lg-4 d-none d-xl-block right-ads-block">
                        @if (is_plugin_active('ads'))
                            @foreach (get_ads_keys_from_shortcode($shortcode) as $key)
                                {!! display_ad($key) !!}
                            @endforeach
