    ...
    <div class="row">
        <div class="col-lg-8 mb-40">
            ...
            <div class="d-flex justify-content-between">
                @if (count($products) > 0)
                    <h6 class="text-body">{!! clean(__('Có :total sản phẩm trong giỏ hàng', ['total' => '<span class="text-brand">' . count($products) . '</span>'])) !!}</h6>
                @endif
            </div>
        </div>
    </div>
                                        ...
                                        <tr class="pt-30">
                                            ...
                                            <td class="price" data-title="{{ __('Price') }}">
                                                @php
                                                    ...
                                                    $tax_amount = $cartItem->price * ($tax_rate/100);
                                                @endphp
                                                <h4 class="text-body">{{ format_price($cartItem->price + $tax_amount) }} </h4>
                                                @if ($product->front_sale_price != $product->price)
                                                    <small><del>{{ format_price($product->price+$tax_amount) }}</del></small>
                                                @endif
                                            </td>
                                            ...
                                            <td class="price" data-title="{{ __('Price') }}">
                                                <h4 class="text-brand">{{ format_price(($cartItem->price+$tax_amount) * $cartItem->qty) }} </h4>
                                            </td>
                        ...
                        @if (Cart::instance('cart')->count() > 0)
                            <div class="col-lg-7">
                                <div class="p-0">
                                    ...
                                    <p class="mb-30"><span class="font-lg text-muted">Bạn muốn sử dụng mã giảm giá?</p>
...
{{--                                    @endif--}}
                        ...
                        <button type="submit" name="checkout" class="btn mb-20 w-100">Thanh toán <i class="fi-rs-sign-out ml-15"></i></button>
                    </div>
                </div>
            @endif
        </div>
